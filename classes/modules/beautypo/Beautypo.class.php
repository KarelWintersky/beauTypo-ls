...
class PluginBeautypo_ModuleBeautypo extends Module {
    ...
    var $picture_regexp = "^(https?\:\/\/.*\.(jpe?g|gif|png))( +(.+)?)?";
    var $gallery_picture_regexp = "(^(https?\:\/\/.*\.(jpe?g|gif|png))( +(.+)?)?\n){2,}";

var $youtube_regexp = '~(?:http|https|)(?::\/\/|)(?:www.|)(?:youtu\.be\/|youtube\.com(?:\/embed\/|\/v\/|\/watch\?v=|\/ytscreeningroom\?v=|\/feeds\/api\/videos\/|\/user\S*[^\w\-\s]|\S*[^\w\-\s]))([\w\-]{11})[a-z0-9;:@?&%=+\/\$_.-]*~i';
    // var $youtube_regexp = "^https?\:\/\/(www\.)?youtu(\.be|be\.com).*(\/|\?|=)([\w-]{11})((&|\?).+)?$";
    var $vimeo_regexp = "^https?:\/\/[^\d]*([\d]{2,11})?";
